...
2025 年是个人 AI 助手的爆发前夜,市面上的助手都关在各自的 app 里,而聊天应用是所有人已经打开着的入口。OpenClaw 这个名字还不存在,它只是 Peter Steinberger 用大约一小时搭出的 WhatsApp 助手原型,把 Claude 接进聊天应用,让一个 AI 能替他处理消息。原型管用,他决定开源,名字叫 Clawd,是对 Claude 的谐音。这个取名在两个月后成为风暴的种子,但在 11 月,它只是一个小项目的俏皮话。

原型期最致命的问题在消息通道本身。WhatsApp 会话会卡死,看门狗超时不够用;AI 自己的回复会再次触发自己,形成回声循环。通道是唯一的入口,通道死了助手就死了,这是整个项目的命门。

应对是可靠性三件套,会话自愈加看门狗超时调到 30 分钟,保证通道不死;回声检测解决自我循环;边发布边打磨,开源当天连发四个版本,用版本节奏倒逼可靠性。
...
开源三周,star 开始爬升,第一批陌生人的 PR 进来了。2 人团队、11 位活跃作者,项目第一次要面对"别人的使用",被迫从原型转向产品。12 月 2 日版本已经推到 v1.3.0,产品化的速度与诞生月一样快。

2 人团队处理不了 10 倍的需求。图片语音被当裸数据收发,文档只有一份配置文件,版本号在 v1.2 到 v1.3 之间跳。每个缺口都是原型期不用管、产品期必须管的事。

三件套按优先级补,媒体链路按 MIME 嗅探处理,文档体系从 claude-config.md 改名建立,版本节奏固定下来。仓库从 steipete/clawdis 迁向 openclaw,品牌同步建立,社区从这章起成为变量。
...
一月是爆火后的失控期。作者数从 11 跳到 304,star 暴涨把 300 多位新作者带进仓库,项目第一次面对"社区规模超过管理能力"。导火索是品牌,Clawd 谐音 Claude 在爆火之后终于引来 Anthropic 法务的注意,1 月 27 日商标争议落地,项目被迫改名,一周之内从 Clawdbot 改到 Moltbot,1 月 29 日定名 OpenClaw。

两个危机同时爆。品牌危机里,改名窗口成了骗局的真空期,冒充项目的假 $CLAWD 代币骗局冲到 1,600 万美元市值,社区被收割了一轮。工程危机里,300 人社区没有 CI 护栏,日更节奏没有版本纪律,合入靠自觉。
...
改名风暴把项目推到舆论中心,也把创始人推到了聚光灯下。Meta 的扎克伯格亲自接触,OpenAI 的 Altman 称他"天才",项目站在个人项目和大公司之间,Steinberger 说"想改变世界,而不是开一家大公司"。

这个阶段有两个绕不开的问题。一个是风险全押在创始人一人身上,50.9% 的主线提交握在创始人手里,他离开项目就停;一个是生态风险,2 月 16 日前后安全研究者统计出 ClawHub 上 824 个恶意技能,伪装成加密与效率工具窃取信息,供应链阴影第一次落到技能生态头上。

创始人问题用治理解决,2 月 14 日宣布加入 OpenAI,项目转为 OpenAI 赞助的独立开源基金会。安全问题这个月只完成了报警,方案要等 4 月的签名机制,先让社区知道危险存在。
...
基金会治理落地后的第一个完整月,团队与社区同时全速运转,技能生态从社区自发变成官方治理。技能市场在 2 月的恶意技能警报后没有减速,反而继续膨胀,MoltHub 达到 8,000 多个技能、20 万月活。

问题在入口。市场入口握在第三方手里,官方对"装什么技能"没有控制,野蛮生长没有刹车。官方需要把"装技能"这个动作抓回自己手里,3 月的版本就是答案。

v2026.3.22 一次发布 45 个新功能、13 个破坏性变更,ClawHub 原生接进 openclaw plugins install,技能市场从网站变成命令;48 小时默认 agent 超时、SSH 与 OpenShell 沙箱后端,执行环境开始受控;旧品牌痕迹(.moltbot 目录、CLAWDBOT_/MOLTBOT_ 环境变量)一次清完,改名风暴的尾巴在这个月扫尽。
...
4 月的版本把签名机制装进安装链路,clawmanifest.json 用 SHA256 钉死文件路径、网络端点与 shell 命令,内核级 eBPF 强制杀掉脚本外的进程,供应链安全从"提醒"变成"强制"。
...
安全加固没有减速开发,5 月的提交量冲到九个月最高。16,537 条主线提交、86 个 tag,双双创下纪录,项目同时面对开发效率的天花板,人力是有限的,功能需求还在涨,传统的"人写代码"模式见顶。

这个阶段的应对是用 AI 代理开发 AI 代理,Steinberger 带进 OpenAI 的团队用约 100 个并发 AI 编码代理开发 OpenClaw,每月烧掉约 130 万美元的 API 额度。这个模式在这个月公开,"龙虾之父烧钱"成了科技媒体标题,但它确实撑住了峰值。同期 Moltbook 上线,纯 AI agent 组成的社交网络开始运转,dancesWithClaws 的 "Logan" 每小时心跳发帖。
...
九个月的功能爆炸留下了一批"先能用、后修好"的债,焦点从扩张转向加固。7 月 PR 编号突然跳到 #87xxx 和 #116xxx,编号池的跳跃说明合入管线经历过大迁移,基建工作与管线重构交织在一起。

三个系统性问题集中暴露,通话链路(OpenAI 实时语音认证、中继断连、Discord 语音保留)不稳定,本地推理(llama.cpp)生命周期没人管,启动并发(迁移 lease)会互相打架,都是峰值流量下欠下的工程债。

逐项还债。7 月社区成员直接修核心通道(PR #87273),通话中继断连清理(#116684);8 月 Discord 实时语音保留上限、llama.cpp 生命周期修复、启动迁移 lease 并发修复(#120959,最新的主线提交)。版本节奏没有停,v2026.6.34 滚到九个月第 233 个 release。
