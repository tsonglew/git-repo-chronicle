...
阶段定位。消息入口的个人 AI 助手原型期。项目做的事一句话能说清,让 Claude 通过 WhatsApp 替人处理消息。它没有产品化野心,只有一个奥地利开发者的一小时原型和"也许有人需要"的直觉。

核心问题。消息通道不可靠。WhatsApp 会话会卡死,AI 自己的回复会再次触发自己形成回声循环,看门狗超时不够用。通道是唯一的入口,通道死了助手就死了,这是原型期最致命的问题。

解决方案。三件套,会话自愈加看门狗超时调到 30 分钟,保证通道不死;回声检测解决自我循环;边发布边打磨,开源当天连发四个版本,用版本节奏倒逼可靠性。

背景与动机。2025 年是个人 AI 助手的爆发前夜,市面上的助手都关在各自的 app 里,而聊天应用是所有人已经打开着的入口。Peter Steinberger 用大约一小时搭出原型,把 Claude 接进 WhatsApp,让一个 AI 能替他处理消息。原型管用,他决定开源。名字 Clawd 是对 Claude 的谐音,这个取名在两个月后成为风暴的种子,但在 11 月它只是一个小项目的俏皮话。
...
阶段定位。从原型到产品的过渡期。2 人团队,11 位作者,开源三周后第一批陌生人的 PR 进来,项目第一次要面对"别人的使用"。

核心问题。2 人团队处理不了 10 倍的需求。图片语音被当裸数据收发,文档只有一份配置文件,版本号在 v1.2 到 v1.3 之间跳,没有体系。每个缺口都是原型期不用管、产品期必须管的事。

解决方案。三件套按优先级补,媒体链路按 MIME 嗅探处理,文档体系从 claude-config.md 改名建立,版本节奏固定下来(12 月 2 日推到 v1.3.0)。仓库从 steipete/clawdis 迁向 openclaw,品牌同步建立。

背景与动机。开源三周,star 开始爬升,第一批陌生人的 PR 进来了。2 人团队处理不了 10 倍的工作量,项目被迫产品化,媒体要处理、文档要跟上、版本要稳定。12 月 2 日版本已经推到 v1.3.0,产品化的速度与诞生月一样快。
...
阶段定位。爆火后的失控期。一个月里作者数从 11 跳到 304,项目第一次面对"社区规模超过管理能力"。

核心问题。两个同时爆发的危机。品牌危机,Clawd 谐音 Claude 引来 Anthropic 商标争议,1 月 27 日被迫改名,一周内 Clawdbot 改 Moltbot 再改 OpenClaw,改名窗口里假 $CLAWD 代币骗局冲到 1,600 万美元市值。工程危机,300 人社区没有 CI 护栏,日更节奏没有版本纪律。

解决方案。品牌问题用改名切割,代价是失去 Claude 谐音带来的流量红利;工程问题用两道闸,conformance 一致性测试进 CI(PR 从 #3677 打到 #5807),版本制从语义化切到日期制(v2026.1.8 起每天一个 tag)。

背景与动机。1 月初,项目还叫 Clawdbot,Claude 谐音的名字在爆火之后终于引来 Anthropic 法务的注意。1 月 27 日商标争议落地,项目被迫改名;一周之内从 Clawdbot 改到 Moltbot,1 月 29 日定名 OpenClaw。改名窗口里,冒充项目的假 $CLAWD 代币骗局一度冲到 1,600 万美元市值。同一时期,star 暴涨把 300 多位新作者带进仓库,项目第一次面对"社区规模超过管理能力"的问题。
...
阶段定位。治理转型期。创始人站在个人项目和大公司之间,项目从"一个人的项目"变成"基金会的项目"。

核心问题。两个。风险全押在创始人一人身上,50.9% 的主线提交握在创始人手里,他离开项目就停;生态风险,824 个恶意技能在 ClawHub 上伪装成加密与效率工具窃取信息,供应链阴影第一次落到技能生态头上。

解决方案。创始人问题用治理解决,2 月 14 日加入 OpenAI,项目转为 OpenAI 赞助的独立开源基金会,Steinberger 说"想改变世界,而不是开一家大公司"。安全问题只完成了报警(2 月 16 日统计公开),解决方案要等 4 月的签名机制,这个月先让社区知道。

背景与动机。改名风暴把项目推到舆论中心,也把创始人推到了聚光灯下。Meta 的扎克伯格亲自接触,OpenAI 的 Altman 称他"天才"。Steinberger 说"想改变世界,而不是开一家大公司",2 月 14 日宣布加入 OpenAI,OpenClaw 转为 OpenAI 赞助的独立开源基金会。同月,安全研究者统计出 ClawHub 上 824 个恶意技能,伪装成加密与效率工具窃取信息,供应链阴影第一次落到技能生态头上。
...
阶段定位。平台期。技能生态从社区自发变成官方治理,生态入口的争夺是这个月的主题。

核心问题。技能市场失控的风险。MoltHub 达到 8,000 多个技能、20 万月活,入口在第三方手里,2 月的恶意技能警报没有减速野蛮生长。官方需要把"装技能"这个动作抓回自己手里。

解决方案。v2026.3.22 把 ClawHub 原生接进 openclaw plugins install,技能市场从网站变成命令;48 小时默认 agent 超时、SSH 与 OpenShell 沙箱后端,执行环境开始受控;旧品牌痕迹(.moltbot 目录、CLAWDBOT_/MOLTBOT_ 环境变量)一次清完。

背景与动机。基金会治理落地后的第一个完整月,团队与社区同时全速运转。技能生态在 2 月的恶意技能警报后没有减速,反而继续膨胀,MoltHub 达到 8,000 多个技能、20 万月活。官方需要把市场抓回自己手里,3 月的版本就是答案。
...
阶段定位。供应链安全制度化。从"提醒用户小心"到"技术上强制可信"。

核心问题。技能是脚本,脚本能执行任意命令。市场越大,供应链风险越大,824 个恶意技能的警报需要一个代码层答案,不能靠社区自觉。

解决方案。v2026.4.12 给技能安装链路装上三层锁,clawmanifest.json 用 SHA256 钉死文件路径、网络端点与 shell 命令,内核级 eBPF 强制杀掉脚本外的进程。供应链安全从"提醒"变成"强制"。发布节奏同步转型,从日更进入周更加预发布(v2026.4.9 到 v2026.5.16-beta.7)。

背景与动机。2 月的 824 个恶意技能警报需要一个代码层答案。技能是脚本,脚本能执行任意命令,市场越大,供应链风险越大。4 月的版本把签名机制装进安装链路,文件路径、网络端点、shell 命令全部用 SHA256 钉死在清单里,内核级 eBPF 强制杀掉脚本外的进程。
...
阶段定位。规模巅峰期。提交量、版本数、媒体报道三线全高,项目同时面对开发效率的天花板。

核心问题。开发效率跟不上规模。16,537 条主线提交、86 个 tag 是九个月最高,但人力是有限的,功能需求还在涨,传统的"人写代码"模式见顶。

解决方案。用 AI 代理开发 AI 代理,Steinberger 带进 OpenAI 的团队用约 100 个并发 AI 编码代理开发 OpenClaw,每月烧掉约 130 万美元的 API 额度。这个模式在这个月公开,"龙虾之父烧钱"成了科技媒体标题,但它确实撑住了峰值。

背景与动机。安全加固没有减速开发,5 月的提交量冲到九个月最高。Steinberger 带进 OpenAI 的团队用约 100 个并发 AI 编码代理开发 OpenClaw,每月烧掉约 130 万美元的 API 额度,细节在这个月公开。同时 Moltbook 上线,纯 AI agent 组成的社交网络开始运转,社区 agent 案例 dancesWithClaws 的 "Logan" 每小时心跳发帖。
...
阶段定位。基建还债期。九个月的功能爆炸留下了一批"先能用、后修好"的债,焦点从扩张转向加固。

核心问题。三个系统性问题,通话链路(OpenAI 实时语音认证、中继断连、Discord 语音保留)不稳定,本地推理(llama.cpp)生命周期没人管,启动并发(迁移 lease)会互相打架。都是峰值流量下暴露的工程债。

解决方案。逐项还债,7 月社区成员直接修核心通道(PR #87273),通话中继断连清理(#116684),Discord 实时语音保留上限(8 月),llama.cpp 生命周期修复,启动迁移 lease 并发修复(#120959,最新的主线提交)。版本节奏没有停,v2026.6.34 滚到九个月第 233 个 release。

背景与动机。九个月的功能爆炸留下了一批必须还的基建债,通话中继、实时语音、本地推理、启动并发,每一个都是"先能用、后修好"的典型。7 月 PR 编号突然跳到 #87xxx 和 #116xxx,编号池的跳跃说明合入管线经历过大迁移,基建工作与管线重构交织在一起。
